...
import { ComponentExample, ComponentPreview, ComponentSource } from '~/components/ui'
import { Event } from '~/lib/events'
import { Alert, AlertDescription, AlertTitle, AspectRatio } from '../../../packages/_oldstuff_refactor/ui'
...
const components = {
  ...
  code: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => {
    return (
      <code
        className={cn('relative rounded-sm bg-muted px-[0.3rem] py-[0.2rem] font-mono text-sm', className)}
        {...props}
      />
    )
  },
