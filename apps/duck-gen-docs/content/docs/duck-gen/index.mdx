...
Example (write to multiple targets):

```toml
[extensions.apiRoutes]
outputPath = ["./client/src/types/duck-gen-api-routes.d.ts", "./shared/src/types/duck-gen-api-routes.d.ts"]

[extensions.messages]
outputPath = "./client/src/types/duck-gen-messages.d.ts"
```

Paths resolve relative to `duck-gen.toml`. When you provide a list, Duck Gen
writes the same output to each path.
