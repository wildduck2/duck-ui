...
1. Reads `duck-gen.json` from your project root (or uses defaults).
...
Duck Gen reads `duck-gen.json` from your project root. Paths are resolved
...
- `includeNodeModules` (boolean, default: `false`): Include `node_modules` in
  scanning.
- `outputSource` (string | string[]): Default output directories for generated
  files when a feature does not define its own output source.
...
- `normalizeAnyToUnknown` (boolean, default: `true`): Converts `any` request
  types to `unknown`.
- `outputSource` (string | string[]): Output file path(s) or directories for
...
Override output locations with `extensions.apiRoutes.outputSource` or
`extensions.messages.outputSource`, or use `extensions.shared.outputSource`
...
- **Missing types**: Check `sourceGlobs` and `tsconfigPath` in `duck-gen.json`.
