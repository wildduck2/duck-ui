...
outputPath = ["./client/src/types/duck-gen-api-routes.d.ts", "./backend/src/common/types/duck-gen-api-routes.d.ts"]

[extensions.messages]
enabled = true
outputPath = ["./client/src/types/duck-gen-messages.d.ts", "./backend/src/common/types/duck-gen-messages.d.ts"]
```

## Link the client to the backend

Duck Gen reads your NestJS controllers + DTOs (via `duck-gen.toml`) and generates `ApiRoutes`. Duck Query then uses
...
If you customized `outputPath`, import types from those files instead of `@gentleduck/gen/nestjs`.
...
```tsx title="client/auth.react-query.ts"
