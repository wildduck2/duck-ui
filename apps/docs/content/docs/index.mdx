...
DuckUI draws inspiration from the excellent work done by the **[Shadcn](https://ui.gentleduck.org/)** team, whose library has been a go-to for many developers, including myself. However, while [Shadcn](https://ui.gentleduck.org/) provides a solid foundation, DuckUI takes things to the next level by introducing unique design philosophies and additional features that reflect my personal style and vision for modern UI development.

I want to emphasize that DuckUI is not about reinventing the wheel or copying someone else’s work—it's about building upon the strong base that [Shadcn](https://ui.gentleduck.org/) has established. I have deep respect for the [Shadcn](https://ui.gentleduck.org/) team and the community that has grown around their library. Their work has significantly influenced DuckUI, and I openly acknowledge that inspiration.

The journey from using [Shadcn](https://ui.gentleduck.org/) to creating DuckUI was driven by a desire to explore different approaches, experiment with new ideas, and address specific needs that I encountered in my own projects. While [Shadcn](https://ui.gentleduck.org/) is versatile and powerful, I saw opportunities to enhance the user experience by adding features, fine-tuning performance, and incorporating design elements that align with my personal aesthetic and functional preferences.
...
Here are a few ways DuckUI builds upon the foundation laid by [Shadcn](https://ui.gentleduck.org/):
...
4. **Expanded Component Library**: In addition to the core components inspired by [Shadcn](https://ui.gentleduck.org/), DuckUI introduces new, innovative components that address more specific use cases, helping you build more complex and interactive UIs with ease.
...
While DuckUI is an independent project with its own goals and vision, it's important to acknowledge the influence of [Shadcn](https://ui.gentleduck.org/). Their work has been an invaluable resource and a major inspiration for many developers, including myself. I view DuckUI as a tribute to the open-source spirit, where innovation builds upon innovation, and where new ideas can emerge from standing on the shoulders of giants.
