      ...
      if (item.type === 'registry:block') {
        ...
        chunks = await Promise.all(
          components.map(async (component, index) => {
            ...
            const attr = component.getAttributeOrThrow('x-chunk').asKindOrThrow(SyntaxKind.JsxAttribute)

            const description = attr.getInitializerOrThrow().asKindOrThrow(SyntaxKind.StringLiteral).getLiteralValue()
            ...
            const containerAttr = component.getAttribute('x-chunk-container')?.asKindOrThrow(SyntaxKind.JsxAttribute)
            ...
            children.forEach((child) => {
              ...
              if (importLine) {
                ...
                const newImports = importLine.isDefault ? importLine.text : new Set([...imports, child])

                componentImports.set(importLine.module, importLine?.isDefault ? newImports : Array.from(newImports))
              }
            })
            ...
            const targetFilePath = path.join(cwd(), `registry/${style.name}/${type}/${chunkName}.tsx`)
